...
Zero breaking changes, many new features.
...
First, try the method above. If you still don't have autocomplete in your monorepo, try the guide below:

If you're in a monorepo, you should put your type declaration file at the root of your monorepo. I don't actually know how much this matters, but (I think) it solved some inconsistency bugs for me in my giant monorepo.

1. Create a declarations file

In the root of your monorepo, create a file called `dripsy.d.ts`, and place these contents inside:

```ts
import 'dripsy'

// change this to properly import your theme!
...
Make sure you update the import path to wherever your `theme` variable is, using a relative path directly to it.

And remember, your `theme` variable should be wrapped in `makeTheme`, like this:
...
2. Reference your declaration file in each app

Any subpackage that has its own `tsconfig.json` (likely each application folder) needs to know to use those declarations.

And so, in **each app folder in your monorepo,** create a file called `dripsy-env.d.ts`, and put this inside:

```ts
/// <reference types="../../dripsy.d.ts" />
```

Make sure the path to your `dripsy.d.ts` file from step 1 is correct! Otherwise this will not work.

3. Check if types are working

Try writing `useDripsyTheme().theme.colors`, hover over colors, and make sure it's showing values from your theme. If so, you should be good to go.
-->
